...
Erc20Watcher.prototype.start = function () {
    ...
    return async.waterfall([function (callback) {
        ...
        self.node.services.onixd.on('tip', self._rapidProtectedUpdateTip.bind(self));
        ...
        return async.eachSeries(block.txids, function (txHash, callback) {
            ...
            return self.node.getJsonRawTransaction(txHash, function (err, transaction) {
                ...
                transaction.vout.forEach(function (vout) {
                    if (vout.scriptPubKey && ['call', 'create', 'call_sender', 'create_sender'].indexOf(vout.scriptPubKey.type) !== -1) {
                ...
                var createVouts = transaction.vout.filter(function (vout) {
                    return vout.scriptPubKey && (vout.scriptPubKey.type === 'create' || vout.scriptPubKey.type === 'create_sender') && ContractsHelper.isErc20Contract(vout.scriptPubKey.hex);
                });

                var callVouts = transaction.vout.filter(function (vout) {
                    return (vout.scriptPubKey.type === 'call'  || vout.scriptPubKey.type === 'create_call');
                });
...
Erc20Watcher.prototype.processCreate = function (blockHeight, txHash, receipt, createVouts, callback) {
    ...
    return async.waterfall([function (callback) {
        ...
        return async.eachOfSeries(createVouts, function (vout, voutCreateIdx, callback) {
            if (vout.scriptPubKey && (vout.scriptPubKey.type === 'create' || vout.scriptPubKey && vout.scriptPubKey.type === 'create_sender') && ContractsHelper.isErc20Contract(vout.scriptPubKey.hex)) {
